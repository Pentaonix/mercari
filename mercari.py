# Mercari installment policy

value = input("Enter Figure price:\n")
date_make_decision = input("Enter the day \
    you would buy figure (from 1 to 30):\n")
date_to_pay_each_month = input("Enter the day you make payment \
    for each month (from 1 to 30):\n")
...
# first payment
first_interest = int(
    fig_price * (days_in_a_month - (bought_day + 1) + 1) *
    commission_rate / days_in_a_year
)

months = 1
fig_price = int(
    fig_price - (int(amount)) + first_interest
)
...
while fig_price != 0:
    ...
    interest = int(
        (int(value) * const_day * commission_rate / days_in_a_year) +
        (fig_price * (days_in_a_month - (const_day + 1)) *
            commission_rate / days_in_a_year)
    )
